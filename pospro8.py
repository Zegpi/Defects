...
# read in solution vector as a numpy array
	...
	#uniform = lambda U: linspace(U[0], U[-1], 4*num-3)		#This takes 4 samples per element
	uniform = lambda U: linspace(U[0], U[-1], num)			#This takes 1 sample per element
	...
	VTK().write(outfile,       # output filename
			...
			scalars={nombre+"(1)":0,nombre+"(2)":1,nombre+"(3)":2,nombre+"(4)":3,nombre+"(5)":4,nombre+"(6)":5,nombre+"(7)":6,nombre+"(8)":7}) # adds a scalar plot to the VTK file